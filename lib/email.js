...
export async function sendPriceDropAlert(
  ...
  try {
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/1a998204-ec27-4d8f-b73e-fc9e0bf28785',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'email.js:4',message:'Sending price drop alert',data:{userEmail:userEmail,productId:product?.id,productName:product?.name,oldPrice:oldPrice,newPrice:newPrice,hasApiKey:!!process.env.RESEND_API_KEY,hasFromEmail:!!process.env.RESEND_FROM_EMAIL},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
    // #endregion
            ...
            <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
              ...
              <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 15px; margin: 20px 0; border-radius: 4px;">
                <p style="margin: 0; font-size: 14px; color: #92400e;">
                  <strong>Price dropped by ${percentDrop}%!</strong>
                </p>
              </div>
    ...
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/1a998204-ec27-4d8f-b73e-fc9e0bf28785',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'email.js:100',message:'Resend API response',data:{userEmail:userEmail,hasError:!!error,error:error,hasData:!!data,data:data},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
    // #endregion
    if (error) {
    ...
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/1a998204-ec27-4d8f-b73e-fc9e0bf28785',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'email.js:107',message:'Email sent successfully',data:{userEmail:userEmail},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
    // #endregion
    return { success: true };
  } catch (error) {
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/1a998204-ec27-4d8f-b73e-fc9e0bf28785',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'email.js:110',message:'Email send exception',data:{userEmail:userEmail,error:error?.message,errorStack:error?.stack},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
    // #endregion
